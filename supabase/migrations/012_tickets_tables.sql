...
-- ============================================
-- 4. TICKET_SALES TABLE (Sales to buyers)
-- ============================================
CREATE TABLE IF NOT EXISTS ticket_sales (
    ...
    -- Buyer info (stored directly, no FK to customers)
    buyer_name TEXT,
    buyer_phone TEXT,
    buyer_email TEXT,
...
CREATE INDEX idx_ticket_sales_status ON ticket_sales(status);
CREATE INDEX idx_ticket_sales_buyer ON ticket_sales(buyer_name);
CREATE INDEX idx_ticket_sales_created ON ticket_sales(created_at DESC);
...
-- SELECT: All authenticated users can view
CREATE POLICY ticket_sales_select ON ticket_sales FOR SELECT TO authenticated USING (true);
...
-- UPDATE: Admins/Managers can update all, others only their own
...
COMMENT ON TABLE ticket_sales IS 'Ticket sales to buyers - tracks payment and delivery';
...
COMMENT ON FUNCTION get_ticket_sold_count(UUID) IS 'Returns total sold quantity for a ticket lot (excluding cancelled)';
...
COMMENT ON COLUMN ticket_sales.delivery_method IS 'How tickets are delivered (email, hand, courier, digital, will_call)';
